...
# ==========================================
# 0. 초기 설정 및 API 키/설정 관리
# ==========================================
st.set_page_config(page_title="MealKitChat", layout="wide")
...
if api_key:
    try:
        ...
        st.warning("⚠️ 저장된 API 키가 유효하지 않습니다. 파일을 삭제하고 다시 요청합니다.")
...
if not api_key:
    ...
    with col1:
        st.info("👋 환영합니다! Google API 키가 필요합니다.")
        st.markdown("[Google AI Studio](https://aistudio.google.com/app/apikey)에서 무료로 발급받을 수 있어요.")
    
    input_key = st.text_input("Google AI Studio API Key:", type="password", placeholder="AI... 로 시작하는 키를 붙여넣으세요")
    
    if st.button("🚀 시작하기"):
        if input_key.strip():
            save_api_key(input_key.strip())
            st.success("키 저장 완료! 새로고침합니다.")
            time.sleep(0.5)
            st.rerun()
        else:
            st.error("빈 칸입니다. 키를 입력해주세요.")
    st.stop() 
